...
const ArticleList: React.FC<ArticleListProps> = ({ articles, latestNews }) => {

  const firstTwoArticles = [...articles.slice(0, 2)]; // Get the first 4 articles
  const NextTwoArticles = [...articles.slice(2, 4)]; // Get the first 4 articles
  const remainingArticles = [...articles.slice(4)]; // Get the remaining articles
    ...
    <div>
      <div className='main-grid'>
        <ul className='main-box'>
          {firstTwoArticles.map((article: any, index: number) => (
            <ArticleItem key={index} article={article} />
          ))}
        </ul>
        <ul className='main-box3'>
          {NextTwoArticles.map((article: any, index: number) => (
            <ArticleItem key={index} article={article} />
          ))}
        </ul>
